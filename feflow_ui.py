...
from ThirdParty.FeatureFlow.feature_flow_interface import CheckResolution, Resolution720p, Resolution360p, InformationGetters, FrameRate
...
class FeFlowApp(wx.App):
	...
	def init_frame(self):
		...
		self.mainFrame.Bind(wx.EVT_MENU, self.onQuit, id=xrc.XRCID('exitMenuItem'))

		self.ogFPS = xrc.XRCCTRL(self.mainFrame, 'ogFPSTextCtrl')
		self.mainFrame.Bind(wx.EVT_BUTTON, self.onSelectFile, id=xrc.XRCID('ogFPSTextCtrl'))

		self.interpFPS = xrc.XRCCTRL(self.mainFrame, 'interpFPSTextCtrl')
		self.mainFrame.Bind(wx.EVT_CHOICE, self.getChoice, id=xrc.XRCID('interpFPSTextCtrl'))

		self.choice = xrc.XRCCTRL(self.mainFrame, 'interpolationChoice')
		self.mainFrame.Bind(wx.EVT_CHOICE, self.getChoice, id=xrc.XRCID('interpolationChoice'))

		self.videoPicker = xrc.XRCCTRL(self.mainFrame, 'videoFilePicker')
		self.mainFrame.Bind(wx.EVT_FILEPICKER_CHANGED, self.onSelectFile, id=xrc.XRCID('videoFilePicker'))

		self.outputPicker = xrc.XRCCTRL(self.mainFrame, 'outputDirPicker')
		self.mainFrame.Bind(wx.EVT_DIRPICKER_CHANGED, self.onSelectOutputDir, id=xrc.XRCID('outputDirPicker'))
		...
		self.mainFrame.Bind(wx.EVT_TEXT, self.showIterationCount, id=xrc.XRCID('statusBar'))
	...
	def onRunFeatureFlow(self, event):
		...
		if self.inputFile != '' and self.outputPath != '':
			...
			if resolution.getWidth() < int(1280) and resolution.getHeight() < int(720):
				interpolate360 = Resolution360p(self.inputFile, self.interpNum, self.outputPath)
				interpolate360.runFeatureFlow()
				self.completeDialog()
			else:
				interpolate720 = Resolution720p(self.inputFile, self.interpNum, self.outputPath)
	...
	def applicationThreads(self, event):
		...
		# pbar.start()
		# fflow.start()
		runFeatureFlow = threading.Thread(target=self.onRunFeatureFlow, args=(event,))
